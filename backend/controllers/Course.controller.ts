...
export function getALlCourse(req: Request, res: Response) {
  console.log(req.url);
  try {
...
export function getOneCourse(req: Request, res: Response) {
  const { id } = req.params;
  try {
    const result = async () =>
      await prisma.courses.findUnique({ where: { id: parseInt(id) } });
    result().then((DATA) => res.json({ success: true, data: DATA }));
  } catch (error) {
    res.json({ success: false, data: "no data" });
  }
