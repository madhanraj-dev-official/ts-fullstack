...
const columns = [
  ...
  { id: "name", label: "Event Title", minWidth: 100 },
  {
    id: "description",
...
export default function EventDetails() {
  ...
  const readData =async ()=>{
    await axios.get("/api/v2/kalaimahal/sembanarkiol/event").then((d)=>setData(d.data.data))
  }

  const createData =async(Data,Id) =>{
    await axios.post("/api/v2/kalaimahal/sembanarkiol/event",Data)
    await readData()
  }

  const updateData =async (Data,Id)=>{
    await axios.put(`/api/v2/kalaimahal/sembanarkiol/event/${Id}`,{...Data,id:Id})
    await readData()
  }
  const deleteData =async (id)=>{
    await axios.delete(`/api/v2/kalaimahal/sembanarkiol/event/${id}`)
    await readData()
