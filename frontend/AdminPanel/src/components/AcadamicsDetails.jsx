...
export default function AcadimicsDetails() {
  ...
  const readData =async ()=>{
    await axios.get("/api/v2/kalaimahal/sembanarkiol/branch").then((d)=>setData(d.data.data))
  }

  const createData =async(Data,Id) =>{
    const formData =await new FormData()
    formData.append('branchName',Data.branchName)
    formData.append('branchDescription',Data.BranchDescription)
        ...
        <Table stickyHeader aria-label="sticky table">
          ...
          <TableBody>
            ...
            rows.length==0?<TableRow ><TableCell colSpan={columns.length}><Typography textAlign={"center"}>no data</Typography></TableCell></TableRow>:rows
              ...
              .map(({ id, name, description ,align},index) => {
                return (
